    ...
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute top-14 right-0 bg-[#1a2c47] border border-blue-900/50 rounded-md shadow-lg w-48 z-50">
          ...
          <div className="p-2">
             <Link to="/profile" onClick={() => setIsOpen(false)} className="block w-full text-left px-3 py-2 text-sm text-gray-200 hover:bg-blue-800/30 rounded-md transition-colors">
              My Profile
            </Link>
            {user.is_admin && (
                <button
